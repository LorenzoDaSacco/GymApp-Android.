...
public class MainActivity extends Activity {
    ...
    void addSetRowTo(LinearLayout parent,Exercise e,int idx,boolean editing){
        ...
        done.setOnCheckedChangeListener((b,c)->{if(b.isPressed()){s.done=c;store.save();if(c)startRecovery(e,s);else RecoveryNotifications.cancel(this,s.id);}});
    ...
    void startRecovery(Exercise e,Set s){try{String r=e.recovery==null?"":e.recovery.trim();if(r.isEmpty())return;int sec=parseRecovery(r);if(sec<=0)return;Intent in=new Intent(this,RecoveryReceiver.class);in.putExtra("name",e.name);in.putExtra("setId",s.id);PendingIntent pi=PendingIntent.getBroadcast(this,(int)(s.id&0x7fffffff),in,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);AlarmManager am=(AlarmManager)getSystemService(ALARM_SERVICE);am.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,System.currentTimeMillis()+sec*1000L,pi);}catch(Exception ignored){}}
